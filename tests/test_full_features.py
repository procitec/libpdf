...
def test_chapters(load_full_features_pdf):
    ...
    assert (
        chapters[1].content[0].textbox.text.startswith("libpdf allows the extraction")
    )
...
@pytest.mark.skipif(
    ...
    reason="saving figures: ImageMagick not installed on Win",
)
@pytest.mark.parametrize(
    "load_full_features_pdf", [True], indirect=True
)  # save figures
...
def test_smart_header_footer_detection():
    ...
    assert objects.flattened.paragraphs[0].textbox.text.startswith(
        "libpdf allows the extraction"
    )
    ...
    assert (
        smart_objects.flattened.paragraphs[0].textbox.text
        == "1. Chapter title for header"
    )
    assert (
        smart_objects.flattened.paragraphs[12].textbox.text
        == "1. Chapter test for footer"
    )
    assert (
        smart_objects.flattened.paragraphs[13].textbox.text
        == "2. Chapter title for header"
    )
    assert (
        smart_objects.flattened.paragraphs[17].textbox.text
        == "2. Chapter test for footer"
    )
    assert (
        smart_objects.flattened.paragraphs[18].textbox.text
        == "3. Chapter title for header"
    )
    assert (
        smart_objects.flattened.paragraphs[23].textbox.text
        == "3. Chapter test for footer"
    )
    assert (
        smart_objects.flattened.paragraphs[24].textbox.text
        == "4. Chapter title for header"
    )
    assert (
        smart_objects.flattened.paragraphs[29].textbox.text
        == "4. Chapter test for footer"
    )


@pytest.mark.skipif(
    sys.platform.startswith("win"),
    reason="visual debugging: ImageMagick not installed on Win",
)
